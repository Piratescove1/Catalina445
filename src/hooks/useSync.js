...
export function useSync({ inventory, voyages, maintenance, futureProjects, ditchSop, ditchItems, lockerInventory, provItems, provCategories, labels, onRemoteData }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      snap => {
        ...
        onRemoteData(data.inventory, data.voyages, data.maintenance, data.futureProjects, data.ditchSop, data.ditchItems, data.lockerInventory, data.provItems, data.provCategories, data.labels)
  ...
  const push = useCallback((inv, voy, maint, future, dSop, dItems, lockers, prov, provCats, lbls) => {
    if (!configured || !db || ignoreNext.current || pendingSync) return
    if (!navigator.onLine) {
      offlineDirty.current = true
      pendingData.current = { inv, voy, maint, future, dSop, dItems, lockers, prov, provCats, lbls }
      return
    }
  ...
  const resolveSync = useCallback(async (keepLocal) => {
    ...
    if (keepLocal && pendingData.current) {
      const { inv, voy, maint, future, dSop, dItems, lockers, prov, provCats, lbls } = pendingData.current
      setStatus('syncing')
      try {
        await setDoc(doc(db, 'boats', boatId), {
          inventory:       inv      || [],
